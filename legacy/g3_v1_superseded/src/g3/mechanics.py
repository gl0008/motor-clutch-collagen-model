...
def update_spatial_clutches(
    ...
    apply_traction_protocol(clutches, cfg, time)
    return material, anchors, motor_points, speeds


def apply_traction_protocol(clutches: ClutchState, cfg: G3Config, time: float) -> float:
    """Scale an attached clutch field to a declared total traction target.

    The controller is used only in the G2-equivalent loading protocol.  Binding,
    material-point identity, motor kinematics, Bell unbinding, local projection,
    and action--reaction bookkeeping remain unchanged; the controlled quantity is
    the *sum of applied clutch-force magnitudes*.  A target of zero leaves the
    molecular motor-clutch model untouched.
    """
    target = float(cfg.traction_target)
    bound = np.flatnonzero(clutches.bound)
    if target <= 0.0 or bound.size == 0:
        return 0.0
    if not np.isfinite(clutches.traction_start_time):
        clutches.traction_start_time = float(np.nanmin(clutches.binding_time[bound]))
    elapsed = max(0.0, float(time) - clutches.traction_start_time)
    ramp = min(elapsed / cfg.traction_ramp_time, 1.0)
    applied = target * ramp
    if cfg.traction_sectorwise:
        # A spheroid is not one giant adhesion: each ECM-facing patch receives
        # an equal share of the declared collective load as it becomes engaged.
        per_patch = applied / cfg.spheroid_surface_patches
        for sector in np.unique(clutches.sector_id[bound]):
            group = bound[clutches.sector_id[bound] == sector]
            current = float(np.linalg.norm(clutches.force_vector[group], axis=1).sum())
            if current > np.finfo(float).eps:
                clutches.force_vector[group] *= per_patch / current
    else:
        current = float(np.linalg.norm(clutches.force_vector[bound], axis=1).sum())
        if current <= np.finfo(float).eps:
            return 0.0
        clutches.force_vector[bound] *= applied / current
    return applied
