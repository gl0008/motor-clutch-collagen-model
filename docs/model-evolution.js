(function(){
  ...
  function renderMap(){
    groupMap.innerHTML=groups.map((group,index)=>{
      const node=`<button class="generation-node group-${group.id}" type="button" data-group="${group.id}" aria-pressed="${activeGroup===group.id}"><span>${group.label}</span><b>${escapeHtml(group.title)}</b><small>${escapeHtml(group.phase)}</small></button>`;
      if(index===groups.length-1)return node;
      return `${node}<div class="generation-edge"><span class="edge-line">→</span><p>${escapeHtml(group.edge)}</p></div>`;
    }).join('');
  }
  ...
  function selectGroup(groupId,focusFirst=true){
    ...
    renderMap();
    renderStory(groupId==='all'?versions.find(version=>version.id===activeVersion).group:groupId);
    renderFilters();
    renderList();
  ...
  document.addEventListener('click',event=>{
    ...
    if(versionButton){
      ...
      document.querySelectorAll('.version-select').forEach(button=>button.setAttribute('aria-pressed',button.dataset.version===activeVersion));
      renderDetail(activeVersion);
      return;
    }
  });
