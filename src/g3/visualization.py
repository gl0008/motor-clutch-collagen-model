...
def _draw_gloria_frame(
    ...
    *,
    ...
    for sector in np.flatnonzero(lengths > 0.02):
        sector = int(sector)
...
def make_stage_animation(result: G3RunResult, path, fps: int = 10, max_frames: int = 180):
    """Write the G3A causal-chain animation in Gloria's two-card grammar."""
    path = Path(path)
    ...
    fig.text(0.027, 0.958, "Protrusion growth and attachment", ha="left", va="top",
             fontsize=18, fontweight="bold", family="serif", color=INK)
    fig.text(0.523, 0.958, "Force-driven fibre response", ha="left", va="top",
             fontsize=18, fontweight="bold", family="serif", color=INK)
    fig.text(0.967, 0.927, "vectors ×1000 (display only)", ha="right", va="top",
    ...
    def update(frame):
        ...
        _draw_gloria_frame(
            ...
            show_displacement_vectors=True, displacement_scale=1000.0,
        )
