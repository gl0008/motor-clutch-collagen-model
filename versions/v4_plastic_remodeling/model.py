"""V4: load/unload test of stress-activated crosslink turnover and remodeling.
...
class Config:
    ...
    dynamic_fraction: float = 0.50
    crosslink_off_rate0: float = 0.010
    crosslink_force_threshold: float = 0.015
    crosslink_force_scale: float = 0.035
    crosslink_on_rate: float = 0.35
...
class Network:
    def __init__(self, positions, fibers, fixed, cfg):
        ...
        # A deterministic subset is permitted to turn over in the V4 plastic
        # condition.  The rest remain permanent load-bearing controls.
        n_dynamic = int(round(cfg.dynamic_fraction * len(self.crosslinks)))
        if n_dynamic:
            chosen = np.random.default_rng(cfg.seed + 404).choice(len(self.crosslinks), n_dynamic, replace=False)
            for idx in chosen:
                self.crosslinks[int(idx)].dynamic = True
...
# ---------------------------------------------------------------------------
# V4 extension: stress-activated crosslink turnover and rest-state resetting.


def crosslink_force(link, network):
    if not link.bound:
        return 0.0
    pa=network.material_point(link.edge_a,link.alpha_a)
    pb=network.material_point(link.edge_b,link.alpha_b)
    return network.cfg.crosslink_stiffness*float(np.linalg.norm((pb-pa)-link.rest_vector))


def update_dynamic_crosslinks(network, cfg, step, plastic):
    """Kinetic break/re-form rule used only by the plastic V4 condition.

    Bound dynamic links break with a force-accelerated hazard.  An unbound link
    may reform between the same two material neighborhoods; its new rest vector
    is the current separation.  This rest-state reset is a minimal plasticity
    law: it erases stored link strain and can retain deformation after unloading.
    It is not yet a molecular collagen-crosslink calibration.
    """
    if not plastic:return
    for lid,link in enumerate(network.crosslinks):
        if not link.dynamic:continue
        if link.bound:
            force=crosslink_force(link,network)
            activation=max(0.0,force-cfg.crosslink_force_threshold)/cfg.crosslink_force_scale
            koff=cfg.crosslink_off_rate0*math.exp(min(20.0,activation))
            if counter_uniform(cfg.seed+900,step,0,lid,7)<1-math.exp(-koff*cfg.dt):
                link.bound=False;link.broken_count+=1
        else:
            if counter_uniform(cfg.seed+900,step,0,lid,8)<1-math.exp(-cfg.crosslink_on_rate*cfg.dt):
                pa=network.material_point(link.edge_a,link.alpha_a)
                pb=network.material_point(link.edge_b,link.alpha_b)
                link.rest_vector=(pb-pa).copy();link.bound=True;link.reformed_count+=1


def run_remodel_condition(cfg=Config(),plastic=False):
    """Fixed-cell load then unload; only the crosslink law differs."""
    net=make_network(cfg);center=np.zeros(2);nsteps=round(cfg.duration/cfg.dt);every=max(1,round(cfg.sample_interval/cfg.dt))
    keys=("time","positions","alignment","rms_displacement","stretch_energy","bend_energy","crosslink_energy","drag_power","active_force","crosslink_bound","broken_total","reformed_total")
    out={k:[] for k in keys};velocity=np.zeros_like(net.r);active=np.zeros_like(net.r);energy={"stretch":0,"bend":0,"crosslink":0}
    initial_alignment=radial_alignment(net,center)
    for step in range(nsteps+1):
        t=step*cfg.dt
        if step%every==0:
            out["time"].append(t);out["positions"].append(net.r.copy());out["alignment"].append(radial_alignment(net,center))
            disp=net.r-net.r0;out["rms_displacement"].append(float(np.sqrt(np.mean(np.sum(disp[~net.fixed]**2,axis=1)))))
            out["stretch_energy"].append(energy["stretch"]);out["bend_energy"].append(energy["bend"]);out["crosslink_energy"].append(energy["crosslink"]);out["drag_power"].append(float(cfg.bead_drag*np.sum(velocity*velocity)))
            out["active_force"].append(cfg.total_pull_force if t<cfg.load_duration else 0.0);out["crosslink_bound"].append([bool(x.bound) for x in net.crosslinks]);out["broken_total"].append(sum(x.broken_count for x in net.crosslinks));out["reformed_total"].append(sum(x.reformed_count for x in net.crosslinks))
        if step==nsteps:break
        magnitude=cfg.total_pull_force*min(1.0,t/cfg.force_ramp_time) if t<cfg.load_duration else 0.0
        active,_=active_forces(net,center,magnitude,cfg.protrusion_angle_deg,cfg.protrusion_half_width_deg)
        velocity,energy=net.advance(active,center,cfg.dt,True)
        update_dynamic_crosslinks(net,cfg,step,plastic)
    for key in keys:out[key]=np.asarray(out[key])
    out.update({"fibers":[x.tolist() for x in net.fibers],"edges":net.edges,"fixed":net.fixed,"crosslinks":[asdict(x) for x in net.crosslinks],"config":asdict(cfg),"plastic":plastic,"initial_alignment":initial_alignment,"residual_alignment_change":float(out["alignment"][-1]-initial_alignment)})
    return out


def run_remodel_pair(cfg=Config()):
    """Permanent-elastic and plastic runs with identical geometry and loading."""
    return {"elastic":run_remodel_condition(cfg,False),"plastic":run_remodel_condition(cfg,True),"config":asdict(cfg)}
