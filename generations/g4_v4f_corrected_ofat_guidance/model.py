"""G4 v4F: corrected one-factor-at-a-time guidance experiment.

This module preserves G4 v4E and corrects three shared-baseline omissions
before comparing guidance mechanisms:
...
All four cases use identical cell--ECM reaction mechanics, dynamic contact
search, initial contacts, clutch constants and random-stream addressing.  The
cell never shrinks and receives no prescribed world-coordinate polarity.  E0
and E1 have no directional memory at any time.  In E2/E3 only, ``front`` is
created after a real collagen contact survives the maturation time.
...
class G4V4FConfig(G4V4Config):
    """Configuration for the corrected G4 v4F OFAT experiment.
...
def make_radial_tract_spec(
    cfg: G4V4FConfig, base_spec: NetworkSpec
...
def make_matched_specs(cfg: G4V4FConfig = G4V4FConfig()) -> dict:
...
def _weighted_initial_sites(
    candidates: list[ContactPatch], network: Network, count: int, cfg: G4V4FConfig
) -> list[ContactPatch]:
...
if njit is not None:
    ...
    def _uniform(seed, step, channel, item):
        """Counter-addressed 53-bit uniform with independent tuple mixing.
    ...
    def _dynamic_relocate_site(
        ...
        guided, beta_a, beta_m, cone_cos, cell_radius, max_gap, sigma,
    ...
    def _simulate_kernel(
        ...
        # The physical search radius is a shared baseline property.  It must
        # not change between E0/E1 and E2/E3.
        ...
        for step in range(nsteps + 1):
            ...
            if guided and front_site < 0:
                ...
                # A fixed site index would create a hidden world-axis bias
                # because initial sites are placed in angular order.
                winner = -1
                winner_score = -1.0
                ...
                if total_front_weight > 0.0:
                    target = _uniform(counter_seed, step, 11, 0) * total_front_weight
                    cumulative = 0.0
                    for s in range(n_sites):
            ...
            for s in range(n_sites):
                should_search = failures[s] or (
                    not site_available[s] and step % contact_search_steps == 0
                )
                if should_search:
                    contact_searches += 1
            ...
            # Explicit Euler must advance both members of the steric pair from
            # the same t_n geometry.  Moving the cell before evaluating the
            # bead update would apply the cell reaction at t_n but the ECM
            # force at t_{n+1}, creating a small operator-splitting imbalance.
            _advance_numba(
                r, r0, fixed, edges, l0, k_tension, k_compression,
                triplets, curvature0, bend_coefficient,
                link_edge_a, link_edge_b, link_alpha_a, link_alpha_b,
...
def run_v4f_condition(
    cfg: G4V4FConfig = G4V4FConfig(),
    ...
    *,
    ...
    """Run one corrected E0--E3 condition with a common physical baseline."""

    if mode not in MODES:
        raise ValueError(f"unknown guidance mode: {mode}")
    if njit is None:  # pragma: no cover
        raise RuntimeError("G4 v4F long-clock runs require numba")
    ...
    probe = probing_contact_patches(network, np.zeros(2), case_cfg.probing_reach)
    n_sites = min(case_cfg.n_contact_sectors, len(surface), len(probe))
    if n_sites < 4:
        raise RuntimeError("too few natural collagen contacts for v4F")
    # Initial contacts are exactly matched. Guidance may affect relocation only
    # after a front emerges in E2/E3.
    ...
    metrics = {
        ...
        "contact_search_limit": float(case_cfg.contact_width + case_cfg.probing_reach),
        "dynamic_contact_search": True,
        "baseline_directional_memory": False,
